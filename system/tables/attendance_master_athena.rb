...
class ATTENDANCE_MASTER_ATHENA < Athena_Table
...
#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    def x_school_day_ago(x)
        all_school_days = schooldays($config.today)
        i = 1
        while i < x
    ...
    def create_att_master_records
        $students.current_students.each do |sid|
            if !by_studentid(sid)
                ...
                student.new_row("Attendance_Master_Athena").save
                $students.detach(sid)
            end
        end
    end
...
#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    def table
        if !@table_structure
            structure_hash = {
                "name"              => "attendance_master_athena",
                "file_name"         => "attendance_master_athena.csv",
                "file_location"     => "attendance_master_athena",
                "source_address"    => nil,
                "source_type"       => nil,
                "download_times"    => nil,
                "trigger_events"    => true,
