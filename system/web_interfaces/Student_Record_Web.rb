...
#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    def identity
        ...
        a7, b7 = "Age:"             , age
        a8, b8 = "Is Special Ed:"   , s.isspecialed.is_true?    ? "<div style='color:red;'>Yes</div>"   : "No"
        a9, b9 = "Is Active:"       , s.active.is_true?         ? "Yes"                                 : "No"
...
#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    def menu_section
        ...
        output << $field.new("value"=>"Record Requests - Incoming"      ).web.button(:onclick=>"$('#page').val('STUDENT_RECORDS_INCOMING_WEB'           );send_unsaved();clear_student_record_content();send_covered('sid');",    :field_class=>field_class, :no_div=>true ) if $team_member.super_user? || $team_member.rights.module_record_requests.is_true?
        output << $field.new("value"=>"Record Requests - Outgoing"      ).web.button(:onclick=>"$('#page').val('STUDENT_RECORDS_OUTGOING_WEB'           );send_unsaved();clear_student_record_content();send_covered('sid');",    :field_class=>field_class, :no_div=>true ) if $team_member.super_user? || $team_member.rights.module_record_requests.is_true?
        ...
        output << $field.new("value"=>"Tech History"                    ).web.button(:onclick=>"$('#page').val('STUDENT_SALES_FORCE_WEB'                );send_unsaved();clear_student_record_content();send_covered('sid');",    :field_class=>field_class, :no_div=>true ) if $team_member.super_user? || $team_member.rights.module_student_sales_force_case.is_true?
